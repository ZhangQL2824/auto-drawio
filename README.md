...
```bash
# 1. 克隆项目
git clone https://github.com/your-username/auto-drawio.git
cd auto-drawio

# 2. 启动 Draw.io（Docker）
docker run -d -p 8080:8080 --name drawio jgraph/drawio
...
使用 Docker 启动 Draw.io（推荐）：

```bash
docker run -d -p 8080:8080 --name drawio jgraph/drawio
```

验证：访问 `http://localhost:8080` 应该能看到 Draw.io 编辑器
...
**解决方案**：
- ✅ 确认 Draw.io Docker 容器正在运行：`docker ps | grep drawio`
- ✅ 浏览器访问 `http://localhost:8080` 测试
- ✅ 检查浏览器控制台是否有 CORS 错误
- ✅ 尝试重启 Docker 容器：`docker restart drawio`
...
如果您想使用 Docker Compose 一键部署所有服务，可以创建以下配置：
...
services:
  ...
  backend:
    ...
    environment:
      ...
      - DEFAULT_AI_BASE_URL=${AI_BASE_URL}
